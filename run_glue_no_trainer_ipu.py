...
def parse_args():
    ...
    parser.add_argument("--InferencereplicationFactor",
                        dest='InferencereplicationFactor', default=4)
    # parser.add_argument("--OffChipStorage",
    #                     dest='OffChipStorage', action='store_true')
    # parser.add_argument("--deviceIterations",
    #                     dest='deviceIterations', default=4)
...
class ModelWrapper(nn.Module):
    ...
    def __init__(self, args, config):
        ...
        # TODO: bs > 1
        # self.model.config.pad_token_id = np.float32(0)
    ...
    def pipeline_mapping(self):
        ...
        for index, layer in enumerate(self.model.model.decoder.layers):
            ipu = layer_ipu[index+index_offset]
...
def main():
    ...
    opts = get_options(args, train=True)
    infer_opts = get_options(args, train=False)
    ...
    optimizer = AdamW(model.parameters(),
                           lr=args.learning_rate, weight_decay=0.01, eps=1e-6)
    poptorch_model = trainingModel(
            model.train().half(), opts, optimizer=optimizer)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):           
            ...
            # pdb.set_trace()
            loss = poptorch_model(input_ids, attention_mask, labels, eos_index)
            # outputs = poptorch_model(
            #     input_ids = batch['input_ids'],
            #     attention_mask = batch['attention_mask'],
            #     labels = batch['labels']
            # )
