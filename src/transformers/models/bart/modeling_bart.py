...
def shift_tokens_right(input_ids: torch.Tensor, pad_token_id: int, decoder_start_token_id: int):
    ...
    """
    shifted_input_ids = torch.zeros(input_ids.shape,dtype=torch.long)
...
def _expand_mask(mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None):
    ...
    # inverted_mask_shape = inverted_mask.shape
    # inverted_mask = inverted_mask.view(-1)
    # print("START EXPAND")
    min_tensor = torch.full(inverted_mask.shape, torch.finfo(dtype).min)
    return min_tensor*inverted_mask
    # for i in range(len(inverted_mask)):
    #     if not inverted_mask[i]:
    #         min_tensor[i] = inverted_mask[i]
    # print("END EXPAND")
    # return torch.reshape(min_tensor, inverted_mask_shape)
    # min_tensor = torch.reshape(min_tensor, inverted_mask_shape)
...
class BartForSequenceClassification(BartPretrainedModel):
    ...
    #     index_offset += self.model.model.encoder.config.encoder_attention_heads
    #     for index, layer in enumerate(model.model.decoder.layers):
    #         ipu = layer_ipu[index+index_offset]
    #         self.model.model.decoder.layers[index] = poptorch.BeginBlock(
    #             layer, f"Decoder_l{index}", ipu_id=ipu)
    #         print(f"Decoder_l{index}")
    #         print("decoder.layer{} --> IPU {}".format(index, ipu))
    #     index_offset += self.model.model.decoder.config.decoder_attention_heads
    ...
    def forward(
        ...
        # eos_index = eos_index.view(hidden_states.size(0), -1, eos_index.size(-1))[:,-1,:]
        # eos_index = eos_index[:,1]
        # size = input_ids.size(1)
        # for i in range(len(eos_index)):
        #     eos_index[i] += size*i
        sentence_representation = hidden_states.view(-1, hidden_states.size(-1))
        sentence_representation = torch.index_select(sentence_representation, 0, eos_index)

        # sentence_representation = [hidden_states[tns[0], tns[1]] for tns in eos_index]
        # sentence_representation = torch.stack(sentence_representation)
        # sentence_representation = sentence_representation.view(hidden_states.size(0), -1, hidden_states.size(-1))[
        #     :, -1, :
        # ]
        # pdb.set_trace()
